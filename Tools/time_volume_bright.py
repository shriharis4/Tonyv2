...
async def control_system_volume(prompt: str, volume_level: int) -> str:
    """
    Adjusts system volume using simple Windows commands or pycaw.
    """
    try:
        if not 0 <= volume_level <= 100:
            return "⚠️ Error: Volume level must be between 0 and 100."
        
        # 1. Try Pycaw first for instant changes
        if VOLUME_AVAILABLE:
            try:
                ...
                print(f"⚠️ Pycaw set volume failed: {pe}. Trying PowerShell fallback.")
